...
//    }
    NSMutableArray *tempEntries = [NSMutableArray arrayWithContentsOfFile:filepath];
    NSMutableArray *tempEntries1 = [NSMutableArray arrayWithObjects:[tempEntries objectAtIndex:1], nil];//第一个为相关说明
    [tempEntries1 enumerateObjectsUsingBlock:^(NSDictionary *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        NSArray *allKeys = obj.allKeys;
        for (NSString *key in allKeys) {
            NSDictionary *dict = [obj objectForKey:key];
            NSArray *allColors = dict.allValues;
            self.themes = dict.allKeys;
            NSArray *colors = [self colorsFromArray:allColors];
...
- (NSArray *)colorsFromArray:(NSArray *)colors {
    NSMutableArray *result = [@[] mutableCopy];
    for (NSString *number in colors) {
        [result addObject:[self colorFromString:number]];
    }
