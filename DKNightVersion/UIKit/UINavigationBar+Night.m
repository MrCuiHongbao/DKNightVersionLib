...
//cui mark 20170302
- (void)dk_setBackgroundImage:(DKImagePicker)picker forBarMetrics:(UIBarMetrics)barMetrics {
    [self setBackgroundImage:picker(self.dk_manager.themeVersion) forBarMetrics:barMetrics];
//    NSString *key = [NSString stringWithFormat:@"%@", @(barMetrics)];
//    NSMutableDictionary *dictionary = [picker valueForKey:key];
//    if (!dictionary) {
//        dictionary = [[NSMutableDictionary alloc] init];
//    }
//    [dictionary setValue:[picker copy] forKey:NSStringFromSelector(@selector(setBackgroundImage:forBarMetrics:))];
    [self.pickers setValue:[picker copy] forKey:NSStringFromSelector(@selector(setBackgroundImage:forBarMetrics:))];
}
